...
async function buildCartUrlForProducts(products, globalMaxQty = null, cartQtyMode = 'global') {
  ...
  // In tampermonkey mode, we still detect limits server-side but ONLY via fast cart probe
  // (skip slow heuristics like tags, description, product.js parsing).
  // The TM browser script acts as a safety net on /cart page.
  const skipLimitDetection = false; // Always probe — TM mode just uses faster probing
    ...
    if (variant) {
      ...
      if (variant.domain === domain) {
        ...
        if (cartQtyMode === 'global') {
          ...
          if (perProductQty) qty = Math.min(qty, perProductQty);
        } else if (cartQtyMode === 'tampermonkey') {
          // TM mode: use detected store limit, per-product overrides if set
          if (perProductQty) qty = Math.min(qty, perProductQty);
        }
...
/**
 * Probe real checkout limits by creating a Shopify checkout and parsing validation errors.
 * 
 * Unlike cart/add.js probing which uses anonymous sessions (and misses app-enforced limits),
 * this creates an actual checkout which triggers Shopify's checkout validation apps.
 * 
 * Flow:
 * 1. Add items to anonymous cart via /cart/add.js
 * 2. Create checkout via POST /cart (follows redirect to checkout URL)
 * 3. Parse "Must have at most X" errors from checkout page
 * 4. Adjust quantities and return corrected items
 * 
 * @param {string} domain - store domain (e.g. "https://tcgstar.eu")
 * @param {Array} items - [{variantId, quantity, name}]
 * @returns {Array} corrected items with validated quantities
 */
async function probeCheckoutLimits(domain, items) {
  if (!items || items.length === 0) return items;
  
  console.log(`🔍 Checkout limit probe: testing ${items.length} items...`);
  
  const jar = {}; // cookie jar for session
  const headers = {
    'User-Agent': getUA(),
  ...
  try {
    // Step 1: Clear cart
    await axios.post(`${domain}/cart/clear.js`, {}, {
    ...
    // Step 2: Add all items to cart with desired quantities
    for (const item of items) {
      await axios.post(`${domain}/cart/add.js`, 
        { items: [{ id: Number(item.variantId), quantity: Number(item.quantity) }] },
        {
          headers: { ...headers, 'Content-Type': 'application/json' },
          timeout: 8000,
          validateStatus: () => true,
    ...
    // Step 3: Try to access checkout - follow redirects to get the checkout page
    const checkoutRes = await axios.get(`${domain}/checkout`, {
      headers,
      timeout: 15000,
      maxRedirects: 5,
      validateStatus: () => true,
    ...
    const html = typeof checkoutRes.data === 'string' ? checkoutRes.data : '';
    
    // Step 4: Parse "Must have at most X" errors
    ...
    while ((m = re.exec(html))) {
      ...
      if (title && max > 0) {
        limits[title.toLowerCase()] = max;
        console.log(`  🎯 Checkout limit: "${title}" → max ${max}`);
      }
    }

    if (Object.keys(limits).length === 0) {
      console.log('  ✅ No checkout limits found — quantities OK');
      return items;
    }

    // Step 5: Match limits to items by fuzzy title matching
    const corrected = items.map(item => {
      const itemName = (item.name || '').toLowerCase();
      
      // Direct match
      for (const [limitTitle, max] of Object.entries(limits)) {
        if (itemName.includes(limitTitle) || limitTitle.includes(itemName)) {
          if (item.quantity > max) {
            console.log(`  📉 ${item.name}: ${item.quantity} → ${max}`);
            return { ...item, quantity: max, checkoutLimit: max };
          }
        }
      }

      // Token overlap match
      const itemTokens = new Set(itemName.replace(/[^a-z0-9]+/g, ' ').split(' ').filter(Boolean));
      for (const [limitTitle, max] of Object.entries(limits)) {
        const limitTokens = new Set(limitTitle.replace(/[^a-z0-9]+/g, ' ').split(' ').filter(Boolean));
        let common = 0;
        for (const t of itemTokens) if (limitTokens.has(t)) common++;
        const overlap = common / Math.max(itemTokens.size, limitTokens.size, 1);
        if (overlap > 0.5 && item.quantity > max) {
          console.log(`  📉 ${item.name}: ${item.quantity} → ${max} (fuzzy match ${Math.round(overlap*100)}%)`);
          return { ...item, quantity: max, checkoutLimit: max };
        }
      }
    ...
    // Step 6: Clear cart after probing
    try {
      await axios.post(`${domain}/cart/clear.js`, {}, {
        headers: { ...headers, 'Content-Type': 'application/json' },
        timeout: 8000,
    ...
    console.log(`  ⚠️ Checkout probe failed: ${e.message}`);
    return items; // return original items on failure
  }
}

/**
 * Build cart URL with checkout-validated quantities.
 * This is the "smart" builder that:
 * 1. Detects limits via cart/add.js (fast, but unreliable for app-enforced limits)
 * 2. Probes actual checkout for real limits
 * 3. Builds final URL with correct quantities
 */
async function buildSmartCartUrl(products, globalMaxQty = null, cartQtyMode = 'global') {
  // Step 1: Build initial cart with best-guess quantities
  const initial = await buildCartUrlForProducts(products, globalMaxQty, cartQtyMode);
  if (!initial.cartUrl || !initial.items.length || !initial.domain) return initial;

  // Step 2: Probe checkout for real limits
  console.log('🔍 Probing checkout for real limits...');
  const correctedItems = await probeCheckoutLimits(initial.domain, initial.items);

  // Step 3: Rebuild URL with corrected quantities
  const cartUrl = buildCartUrl(initial.domain, correctedItems);
  const checkoutUrl = buildCheckoutUrl(initial.domain, correctedItems);

  const changed = correctedItems.some((item, i) => item.quantity !== initial.items[i]?.quantity);
  if (changed) {
    console.log('✅ Checkout probe found different limits — cart URL updated');
  }
...
module.exports = { getShopifyVariant, buildCartUrl, buildCheckoutUrl, buildCartUrlForProducts, buildSmartCartUrl, probeCheckoutLimits, detectOrderLimit };
